...
hybridExSets <- function(a) {
  tempSdUser <- matchingOrder %>%
    slice(a) %>%
    mutate(exSetSubject=1)
  
  if (tempSdUser$switcher == 0) {
    # Each exposure set has all patients with the same no. of prior prescriptions of the
    ...
    tempComparator <- patientPool %>%
      ...
      filter(compRx==1) %>%
      filter(timeSinceIndex <= timeWindow & rxYear==tempSdUser$rxYear[1] & numberCompRxs==1) %>% 
  ...
  if (tempSdUser$switcher == 1) {
    ...
    tempComparator <- patientPool %>%
      ...
      filter(rxYear== tempSdUser$rxYear[1]) %>%
...
# excluded study drug users
if (debugMode == TRUE) {
  if (file.exists("data/debug_hybridExcludedStudyDrugUsers.csv")) {
    # Load in patient summary
    excludedStudyDrugUsers <- read_csv("data/debug_hybridExcludedStudyDrugUsers.csv") %>%
      mutate_at(c("patid"), as.character)
  } else {
    message("Error: debug_hybridExcludedStudyDrugUsers.csv, not found.")
  }
} else{
  if (file.exists("data/hybridExcludedStudyDrugUsers.csv")) {
    
    excludedStudyDrugUsers <- read_csv("data/hybridExcludedStudyDrugUsers.csv") %>%
      mutate_at(c("patid"), as.character)
  } else {
    message("Error: hybridExcludedStudyDrugUsers.csv, not found.")
  }
}

# load _exSets generated - for exSet Nos
if (debugMode == TRUE) {
  if (file.exists("data/debug_hybridExSets.csv")) {
    # Load in patient summary
    exSetNums <- read_csv("data/debug_hybridExSets.csv") %>%
      mutate_at(c("patid"), as.character) %>% 
      filter(exSetSubject == 1)
  } else {
    message("Error: debug_hybridExSets.csv, not found.")
  }
} else{
  if (file.exists("data/hybridExSets.csv")) {
    
    exSetsNums <- read_csv("data/hybridExSets.csv") %>%
      mutate_at(c("patid"), as.character) %>% 
      filter(exSetSubject == 1)
  } else {
    message("Error: hybridExSets.csv, not found.")
  }
}
...
# Estimate ps for all patients in the exposure set
     ...
     if (comparatorCandidate$exclusionFlag[1] == TRUE) {
       ...
       for (rowComp in 2:nrow(comparatorInfo)) {
         ...
         patientPool <- patientPool %>%
           filter(!patid %in% comparatorCandidate$patid) # they should be drug users in the matching order - so I don't think this matters
         # move out of loop 
         break
         }
